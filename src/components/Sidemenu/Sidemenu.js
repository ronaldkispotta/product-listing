  ...
  const removeValue = (inputVal, filterName) => {
    if (inputVal) {
      setMainState((prev) => {
        ...
        currentValues[filterName] = currentValues[filterName].filter(
          (val) => val !== inputVal
        )
      ...
      <div>
        ...
        {appliedFilters.length > 0 && (
          <div className='appliedFiltes'>
              ...
              return (
                <div key={ele} className='element-container'>
                  {appliedFilters.length > 1 && (
                    <span className='cancel' onClick={() => cancelfunc(ele)}>
                      X
                    </span>
                  )}
            ...
            <button className='clearall' onClick={onclickHandler}>
              clear all
            </button>
          </div>
        )}
