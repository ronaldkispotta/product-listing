...
function App() {
  ...
  useEffect(() => {
    const dataFetching = async () => {
      const api = await fetch(
        `https://pim.wforwoman.com/pim/pimresponse.php/?service=category&store=1&url_key=top-wear-kurtas&page=1&count=20&sort_by=&sort_dir=desc&filter=${filter}`
      )
  ...
  return (
    <div>
      ...
      {data ? (
        <Homepage searchedValue={searchedValue} data={data} />
      ) : (
        <Nodata />
      )}
